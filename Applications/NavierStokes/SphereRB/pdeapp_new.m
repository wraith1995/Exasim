...
Ttop = 0.50;                     % Temperature top surface (equal to Tbot to start)
...
pde.GMRESrestart=20;            % number of GMRES restarts
...
mesh.boundaryexpr = {@(p) abs(p(1,:).^2+p(2,:).^2+p(3,:).^2-R0^2)<1e-4, @(p) abs(p(1,:).^2+p(2,:).^2+p(3,:).^2-R1^2)<1e-4};
